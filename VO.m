...
for i = 1:length(cam0.Files)
    ...
    if ~isempty(features)
        ...
        features.r_pos = features.r_pos(lm(:,2),:);
        ...
        features.r_pos = features.r_pos(rm(:,2),:);
...
%             p  = [p;triangulate(round(temp_l_pos(it).Location),round(temp_r_pos(it).Location),P1,P2)];
        ...
        if isempty(pose)
            ...
            %worldPoints = zeros(size(p));
            %for index = 1:size(p,1)
            %    worldPoints(index,:) = pose.Translation + p(index,:)*pose.R;
            %end
            %pose = rigidtform3d(pose.A*estworldpose(double(temp_l_pos.Location),worldPoints,intrinsics).A);
...
%             matchedPoints2 = currPoints(indexPairs(:, 2))
        ...
        if ~isempty(relativePose)
            ...
            vSet = addConnection(vSet, viewId, viewId-1, relativePose,"Matches",round(temp_l_pos.Location));
            ...
            % IS THE PREVIOUS TRIANGULATION NEEDED NOW? (I think so), I
            % think that we don't even need the triangulation if we use
            % these functions since the est worlpose function would handle
            % all of that for us. It feels a bit like cheating to use these
            % functions when we should implement a VO system. But that is
            % just my opinion
