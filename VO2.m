...
for i = 1:length(cam0.Files)
    ...
    if ~isempty(features)
        ...
        % Draw map in xy-plane
        figure(10)
        plot([temp_old(2) old_translation(2)],[temp_old(1) old_translation(1)])
        title('Travel map')
        xlabel('x coordinate')
        ylabel('z coordinate')
        hold on
...
function [translation, rotation] = calc_rel_pose(points_img, points_world_current, points_world_old)
    ...
    for i = 1:size(points_img, 1)
        ...
        A(3 * i - 0, :) = [-points_world_old(i, 1) * points_img(i, 2) -points_world_old(i, 2) * points_img(i, 2) -points_world_old(i, 3) * points_img(i, 2) -points_img(i, 2) points_world_old(i, 1) * points_img(i, 1) points_world_old(i, 2) * points_img(i, 1) points_world_old(i, 3) * points_img(i, 1) points_img(i, 1) 0 0 0 0];
    end
